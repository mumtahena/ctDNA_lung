...
#png(filename = "Figure 2a.png",width = 14,height = 10,units = "in",res=1024)
waterfall(
  mutationData,
  fileType = "Custom",
...
#png(filename = "Figure2bc.png",width = 8,height = 4,units = "in",res=2048)
...
## Figure 5
```{r Figure 5, echo=FALSE,cache=TRUE}
...
```{r Supplemental.Figure.1,echo=FALSE,cache=TRUE,warning=FALSE}
##Supplemental Figure 1(A)
###histogram of reported VAFs
supp_1a<-ggplot(prep_gg,aes(x=ctDNA))+geom_histogram(color="brown3", fill="brown3")+
  labs(x="ctDNA%", y = "VAF% of variants")+
  theme_classic()+
  font("xy.text",size = 14,color = "black")+font("xylab",size = 10,color = "black")+
  ...
  annotate("text",
    ...
    label=paste(
      "Median: ",
      median(prep_gg$ctDNA, na.rm = T),
      "%",
      "\n",
      "Mean: ",
      round(mean(prep_gg$ctDNA, na.rm = T), digit = 1),
      "%",
      "\n",
      "Min: ",
      min(prep_gg$ctDNA, na.rm = T),
      "%",
      "\n",
      "Max: ",
      round(max(prep_gg$ctDNA, na.rm = T), digit = 1),
      "%",sep = ""
    ))

nccn_gg<-prep_gg[prep_gg$Gene%in%c("ALK","EGFR","ROS1","BRAF","NTRK1","NTRK2","KRAS"),]

supp_1b<-ggplot(nccn_gg, aes(x=Gene, y=ctDNA)) + 
  geom_violin()+labs(x="NCCN Genes", y = "ctDNA%",face="bold")+
  geom_jitter(shape=16, position=position_jitter(0.2))+
  scale_color_brewer(palette="Dark2")+
  theme_classic()+font("xy.text",size = 14,color = "black")+font("xylab",size = 10,color = "black")
 
#png("Supplemental Figure 1",width = 6,height = 8,units = "in",res=1024)
ggarrange(supp_1a,supp_1b,nrow=2,labels = c("A","B"))
#dev.off()
...
```{r Supplemental.Figure.2,include=FALSE,cache=TRUE}
##ctDNA VAF vs TCGA LUAD gene rank based on mutations
ctdna_cases<-aggregate(Patient.ID~Gene,prep_gg, unique)
ctdna_cases$cases<-unlist(lapply(ctdna_cases$Patient.ID,length))
ctdna_cases$perct<- round(ctdna_cases$cases/370*100,digits = 1)



gene_freq<-as.data.frame(table(prep_gg$Gene)[order(table(prep_gg$Gene),decreasing = T)])
tcga<-read.table("frequently-mutated-genes.2020-05-22.tsv",sep = "\t",header = T)##LUAD+ LUSC
names(tcga)<-c("cases","perct_cases")
tcga<-data.frame(tcga[rownames(tcga)%in%ctdna_cases$Gene,])
tcga$perct_cases<-round(tcga$perct_cases,digits = 1)
tcga$Gene<-rownames(tcga)
tcga$Cases.Source<-"TCGA_tissue"
tcga<-tcga[order(tcga$perct_cases),c("Gene","perct_cases","Cases.Source")]

ctdna_cases$Cases.Source<-"COH_ctDNA"
ctdna_cases$perct_cases<-round(ctdna_cases$perct,digits = 1)
ctdna_cases<-ctdna_cases[order(ctdna_cases$perct),c("Gene","perct_cases","Cases.Source")]
ctdna_cases<-data.frame(ctdna_cases[ctdna_cases$Gene%in%rownames(tcga),])

com_ctDNA_tcga<-rbind(tcga,ctdna_cases)
com_ctDNA_tcga$perct_cases<- as.numeric(as.vector(com_ctDNA_tcga$perct_cases))
com_ctDNA_tcga<-com_ctDNA_tcga[order(com_ctDNA_tcga$perct_cases,decreasing = T),]
#Supplemental Figure 2
#
#tiff("Supplemental Figure 2.tiff",width = 6,height = 7,units = "in",res = 1024)
ggdotchart(com_ctDNA_tcga, x = "Gene", y = "perct_cases",
           color = "Cases.Source",                                # Color by groups
           palette = c("brown3","#00AFBB"), # Custom color palette
           sorting = "descending",                       # Sort value in descending order
           add = "segments",                             # Add segments from y = 0 to dots
           add.params = list(color = "lightgray", size = 2), # Change segment color and size
           group = "Gene",                                # Order by groups
           dot.size = 7.8,                                 # Large dot size
           label = round(com_ctDNA_tcga$perct_cases,1),                        # Add mpg values as dot labels
...
```{r Supplemental Figure 3, echo=FALSE,cache=TRUE,warning=FALSE}
kras<-ggsurvplot(
    survfit(Surv(I(
...
single_mut<-(prep_gone[,c("Patient.ID","jlastdate","jfirstdate","Age","Gender","Smoking.Status","firstKRAS","firstPIK3CA","firstERBB2","firstBRAF","dflag")])
...
Age+Gender+Smoking.Status+KRAS+PIK3CA+ERBB2+BRAF,single_mut,
subset=jlastdate > jfirstdate))
 
 #ggarrange(braf[[1]],erbb2[[1]],kras[[1]],pik3ca[[1]],nrow = 2,ncol = 2,labels = c("A","B","C","D"),common.legend = T,legend = "bottom")  
  
 
 # png("Supplemental Figure 3.png",width =8,height=12,units = "in",res = 1024)   
  ggarrange(
    ggarrange(
      ggarrange(kras$plot,kras$table,nrow = 2,heights = c(2,1),legend = "none", align = "v"),
      ggarrange(pik3ca$plot,pik3ca$table,nrow=2,heights = c(2,1),legend = "none",align = "v"),
      ncol = 2,
      labels = c("A","B")),
    mut_one,
    nrow = 2,
